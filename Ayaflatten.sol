...
// File contracts/AyaNFT.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.17;
contract AyaNFT is ERC721, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("AyaNFT", "ANFT") {}
...
// File contracts/AyaToken.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.17;
contract Ayatoken is ERC20, Ownable {
    constructor() ERC20("Ayatoken", "ATK") {}
...
// File contracts/AyaCommunity.sol
...
contract AyaCommunity is Ownable {
    ...
    Proposal[] public executedProposals;
    Proposal[] public deniedProposals;

    mapping(uint256 => mapping(address => bool)) hasVoted;
    mapping(address => bool) ismember;
    mapping(address => uint256) membermap;
    
    event MemberJoined(address indexed memberAddress, MembershipTier tier);
    event MemberUpgraded(address indexed memberAddress, MembershipTier newTier);
    event MemberVoted(address indexed memberAddress, uint256 proposalId, bool vote);
    event ProposalCreated(uint256 proposalId, string description);
    event ProposalClosed(uint256 proposalId, bool isClosed);
    event ProposalExecuted(uint256 proposalId, bool isExecuted);
    event ProposalDenied(uint256 proposalId, bool isDenied);
    event TokensTransferred(address indexed recipient, uint256 amount);

    constructor(address _ayatokenAddress, address _ayaNFTAddress, uint256 _minimumVotes) {
        minimumVotes = _minimumVotes;
        ayatoken = IERC20(_ayatokenAddress);
        ayaNFT = IERC721(_ayaNFTAddress);
    }

    function transferTokens(address _recipient, uint256 _amount) external onlyOwner {
        ayatoken.transfer(_recipient, _amount);
        emit TokensTransferred(_recipient, _amount);
    }
    function rewardMember(uint256 rewardAmount, address _memberAddress) external onlyOwner{
    ...
    function becomeMemberWithTokens() external {
        require(ayatoken.balanceOf(msg.sender) >= 400*1e18, "Not enough Ayatokens");
        ...
        member.ayatokensOwned = ayatoken.balanceOf(msg.sender);
    ...
    function becomeMemberWithNFT() external {
    ...
    function upgradeMembership() external {
        ...
        require(member.tier != MembershipTier.Gold, "Already at the highest level");

        if (ayatoken.balanceOf(msg.sender)>= 1000*1e18) {
                member.tier = MembershipTier.Silver;
                member.ayatokensOwned = ayatoken.balanceOf(msg.sender);
            }
        else if (ayatoken.balanceOf(msg.sender) >= 2000*1e18) {
    ...
    emit ProposalClosed(_proposalId, true);
}
...
function executeProposal(uint256 _proposalId) external onlyOwner {
    ...
    require(proposal.Votes > minimumVotes, "Proposal does not have enough votes");
...
function denyProposal(uint256 _proposalId) external onlyOwner {
    require(_proposalId < proposals.length, "Invalid proposal ID");
    ...
    emit ProposalDenied(_proposalId, true);
}
